...
async def app_lifespan(app: FastAPI):
    try:
        ...
        if _process_pool:
            ...
            _process_pool = None
            activity_logger.info("Process pool shutdown")

app = FastAPI(title="Timetable Generator API", version="3.0.0", lifespan=app_lifespan)
...
@app.get("/front.html")
def front_page():
    return FileResponse(BASE_DIR / "front" / "front.html")
...
def config_js():
    ...
    if not supabase_url or not supabase_publishable_key:
        warn_snippet = (
            'console.warn("Supabase config missing. Add SUPABASE_URL and '
            'SUPABASE_PUBLISHABLE_KEY to your .env file.");\n'
        )

    return Response(
        content=f"window.ENV = {json.dumps(payload)};\n{warn_snippet}",
...
MONITORED_REQUEST_PATHS = {"/subjects", "/generate", "/reload_courses"}
_generation_activity_lock = threading.Lock()
_active_generation_requests = 0
_active_generation_users: Dict[str, int] = defaultdict(int)
...
def get_request_user_email(request: Request) -> str:
    """Extract a user email for monitoring from the request headers."""
    email = (request.headers.get("x-user-email") or "").strip().lower()
    return email or "anonymous"

def set_request_log_context(request: Request, **context: Any):
    """Attach request-specific summary data for concise terminal logs."""
...
def format_request_log_context(request: Request) -> str:
    """Render request context as key=value pairs."""
    log_context = getattr(request.state, "log_context", {})
    if not log_context:
        return ""
...
def begin_generation_activity(user_email: str) -> Tuple[int, List[str]]:
    """Track users currently generating timetables."""
    global _active_generation_requests
    with _generation_activity_lock:
        _active_generation_requests += 1
        if user_email != "anonymous":
            _active_generation_users[user_email] += 1
        return _active_generation_requests, sorted(_active_generation_users.keys())

def end_generation_activity(user_email: str) -> Tuple[int, List[str]]:
    """Remove a user from active generation tracking."""
    global _active_generation_requests
    with _generation_activity_lock:
        _active_generation_requests = max(0, _active_generation_requests - 1)
        if user_email in _active_generation_users:
            _active_generation_users[user_email] -= 1
            if _active_generation_users[user_email] <= 0:
                del _active_generation_users[user_email]
        return _active_generation_requests, sorted(_active_generation_users.keys())

def summarize_active_users(users: List[str], max_users: int = 5) -> str:
    """Show a compact preview of active users in the terminal."""
    if not users:
        return "-"
    preview = users[:max_users]
    suffix = ",..." if len(users) > max_users else ""
    return ",".join(preview) + suffix

@app.middleware("http")
async def concise_request_logger(request: Request, call_next):
    """Log only the useful API activity with timing and user details."""
    path = request.url.path
    should_log = path in MONITORED_REQUEST_PATHS or path.startswith("/staff/")
    if not should_log:
    ...
    try:
        ...
        logger.exception(
            "REQ %s %s status=500 user=%s ip=%s time=%.0fms%s",
            request.method,
            path,
            user_email,
            client_id,
            duration_ms,
            format_request_log_context(request),
        )
        raise

    duration_ms = (time.perf_counter() - started_at) * 1000
    activity_logger.info(
        "REQ %s %s status=%s user=%s ip=%s time=%.0fms%s",
        request.method,
        path,
...
class GodModeTimetableFinder:
    ...
    def find_all_timetables(
        ...
        logger.info(f"🚀 ACTIVATED - Priority Mode: {priority_mode.upper()}")
    ...
    def _check_constraints(self, selection: List[CourseSection], 
        ...
        # Max classes per day constraint (gap-aware: every continuous 2 hrs = 1 class)
        max_per_day = kwargs.get('max_per_day')
        if max_per_day:
            ...
            for section in selection:
                for slot in section.time_slots:
                    slots_by_day[slot.day].append((slot.start_min, slot.end_min))

            for day, slots in slots_by_day.items():
                # Sort by start time
                slots.sort()

                block_start, block_end = slots[0]
                ...
                for start, end in slots[1:]:
                    if start == block_end:
                        ...
                        day_counts[day] += max(1, math.ceil(duration / 120))
...
async def subjects_list(request: Request):
    ...
    set_request_log_context(request, subjects=len(subjects) - 1, sections=total_sections)
    return JSONResponse(subjects)

@app.get("/staff/{subject_code}")
async def get_staff(subject_code: str, request: Request):
    ...
    set_request_log_context(request, subject=subject_code, staff=len(staff))
...
async def generate_timetable(
    ...
    constraints_strictness: str = Form("flexible")
):
    ...
    set_request_log_context(
        ...
        staff_mode=staff_strictness,
        constraint_mode=constraints_strictness,
    )

    active_count, active_users = begin_generation_activity(user_email)
    activity_logger.info(
        "GEN start user=%s subjects=%s page=%s active=%s users=%s",
        user_email,
        len(selected_codes),
        page_num,
        active_count,
        summarize_active_users(active_users),
    )
    ...
    try:
        ...
        set_request_log_context(
            request,
            results=len(timetables),
            search=f"{stats.get('time_elapsed', 0):.2f}s",
        )
    except Exception as e:
        # Log full error but show generic message to user
        logger.error(f"Search failed: {e}", exc_info=True)
        set_request_log_context(request, results=0, search="failed")
        return HTMLResponse(
            f'''
            <div style="text-align:center;padding:40px;background:#0f172a;
            border-radius:12px;border:1px solid #1f2937;">
        ...
        active_count, active_users = end_generation_activity(user_email)
        set_request_log_context(
            request,
            active=active_count,
            active_users=summarize_active_users(active_users),
        )
...
async def reload_courses_endpoint(request: Request):
    """Force reload courses from file."""
    course_cache.clear()
    courses = load_courses(force_reload=True)
    set_request_log_context(request, courses=len(courses))
...
if __name__ == "__main__":
    ...
    activity_logger.info("Timetable Generator started")
    activity_logger.info(f"Rate limiting: {RATE_LIMIT_REQUESTS} req/{RATE_LIMIT_WINDOW}s")
    activity_logger.info(f"CORS Origins: {CORS_ORIGINS}")
    activity_logger.info("=" * 80)

    # Load courses at startup
    logger.info("Loading courses...")
    activity_logger.info("Loading courses...")
    courses = load_courses()
    logger.info(f"Loaded {len(courses)} courses at startup")
    activity_logger.info(f"Loaded {len(courses)} courses at startup")
